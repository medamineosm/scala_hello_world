...
import ScalaCrawler.Messages.{Scrap, ScrapFailure, ScrapFinished}
...
class SiteCrawler(supervisor: ActorRef, indexer: ActorRef) extends Actor {
  ...
  var toProcess = List.empty[URI]

  def receive: Receive = {
    case Scrap(url) =>
      // wait some time, so we will not spam a website
      println(s"[SiteCrawler] waiting... $url")
      toProcess = url :: toProcess
    case `process` =>
      toProcess match {
        case Nil =>
        case url :: list =>
          println(s"[SiteCrawler] site scraping... $url")
          toProcess = list
